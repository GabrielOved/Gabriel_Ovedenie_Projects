...
# user agent required to establish the connection to the API
headers = {
...
class ThermoenergeticIssue:
    ...
    def extract_issue(self):
        ...
        address = re.compile(fr"({self.street_name} - \s*.+?)(?:\s*•|\s*Deficienta)") # search for the specified street based on the html body
    ...
    def email_generation(self):
        try:
            ...
            if self.street_name not in cont: # Check to see if the street is present in the webpage
                msg['Subject'] = f'[Automated Email] No Thermoenergetic issue for the {self.street_name} street' # Replacement of the email Subject if the street is not present on the webpage
                content = (('<br>' + '-'*80 + '<br>') +
